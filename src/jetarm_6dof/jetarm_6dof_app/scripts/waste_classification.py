...
class WasteClassificationNode:
    def __init__(self, node_name, log_level=rospy.INFO):
        ...
        weights = '/home/ubuntu/weights/garbage_classification/garbage_classification.onnx'
        self.yolov5 = YOLOV5(weights, TRT_CLASS_NAMES, 0.85)
        ...
        self.enable_transport_srv = rospy.Service('~enable_transport', SetBool, self.enable_transport_srv_callback)
        self.heart = heart.Heart('~heartbeat', 5, lambda e: self.exit_srv_callback(e))

    def go_home(self):
        if self.target is not None and self.target[-1] == "recyclable_waste":
            time = 2.0
        elif self.target is not None and self.target[-1] == "hazardous_waste":
            time = 1.7
        elif self.target is not None and self.target[-1] == "food_waste":
            time = 1.4
        elif self.target is not None and self.target[-1] == "residual_waste":
            time = 1.0
        else :
            time = 1.0
    ...
    def enable_transport_srv_callback(self, req: SetBoolRequest):
        with self.lock:
            if req.data:
                rospy.loginfo("开启抓取")
    ...
    def done_callback(self, state, result):  # 动作执行完毕回调(callback function when the action is completed)
        ...
        if self.finish_percent == 1:  # 如果完整的完成移动(if the motion is completed in full)
            if self.moving_step == 1:  # 如果完成了夹取(if the grasping is completed)
                # self.moving_step = 2
                self.status = 2
    ...
    def place(self):
        ...
        if self.target[-1] == 'food_waste':
               target_name = 'target_2'
        elif self.target[-1] == 'hazardous_waste':
               target_name = 'target_3'
        elif self.target[-1] == 'recyclable_waste':
            target_name = 'target_4'
        else:
            pass
        target_position = rospy.get_param(os.path.join(POSITIONS_PATH, 'waste_classification'))[target_name]
        ...
        goal.grasp.grasp_approach.z = 0.05  # 放置时靠近的方向和距离(direction and distance of approach during placement)
        goal.grasp.grasp_retreat.z = 0.05  # 放置后后撤的方向和距离(direction and distance of retreat after placement)
        goal.grasp.grasp_posture = 310  # 夹取前后夹持器的开合角度(the opening and closing angle of the grippers before and after grasping)
    ...
    def start_moving(self, pose_t, pose_R):
        ...
        if self.enable_voice:
            play("/home/ubuntu/jetarm/src/jetarm_6dof/jetarm_6dof_app/waste_pcm/" + self.target[-1] + ".pcm")
        while True:
    ...
    def image_process(self):
        ...
        try:
            if self.moving_step == 0 and self.roi is not None and self.K is not None and self.D is not None:
                ...
                if len(cards) > 0 and self.enable_transport:
                    ...
                    if distance(center, center_last) < 50:
                        ...
                        if self.count > 30:
                            ...
                            world_pose[2] = 0.04
                            world_pose = np.matmul(self.white_area_center, xyz_euler_to_mat(world_pose, (0, 0, 0)))  # 转换到相机相对坐标(convert to camera related coordinates)
                            world_pose[2] = 0.04
                            #world_pose = np.matmul(self.white_area_cam, xyz_euler_to_mat(world_pose, (0, 0, 0)))  # 转换到相机相对坐标(convert to camera related coordinates)
                            # pose_end = np.matmul(self.hand2cam_tf_matrix, world_pose)  # 转换的末端相对坐标(relative coordinates of the converted end)
                            # pose_world = np.matmul(self.endpoint, pose_end)  # 转换到机械臂世界坐标(convert to robotic arm's world coordinates)
