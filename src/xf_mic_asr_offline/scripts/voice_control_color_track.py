...
from hiwonder_interfaces.srv import SetInt8
...
class VoiceControlColorTrackNode:
    def __init__(self, name):
        ...
        res = rospy.ServiceProxy('/object_tracking/enter', Trigger)()
        rospy.sleep(2)
        res = rospy.ServiceProxy('/object_tracking/enable_color_tracking', SetBool)(True)
        self.play('running')
        if res.success:
    ...
    def words_callback(self, msg):
        ...
        if words is not None and words not in ['唤醒成功(wake-up-success)', '休眠(Sleep)', '失败5次(Fail-5-times)', '失败10次(Fail-10-times']:
            if words == '追踪红色' or words == 'track red object':
                res = rospy.ServiceProxy('/object_tracking/set_target_color', SetInt8)(1)
                if res.success:
                    self.play('start_track_red')
                else:
                    self.play('track_fail')
            elif words == '追踪绿色' or words == 'track green object':
                res = rospy.ServiceProxy('/object_tracking/set_target_color', SetInt8)(2)
                if res.success:
                    self.play('start_track_green')
                else:
                    self.play('track_fail')
            elif words == '追踪蓝色' or words == 'track blue object':
                res = rospy.ServiceProxy('/object_tracking/set_target_color', SetInt8)(3)
                if res.success:
                    self.play('start_track_blue')
                else:
                    self.play('track_fail')
            elif words == '停止追踪' or words == 'stop tracking':
                res = rospy.ServiceProxy('/object_tracking/enable_color_tracking', SetBool)(False)
                res = rospy.ServiceProxy('/object_tracking/enable_color_tracking', SetBool)(True)
                if res.success:
