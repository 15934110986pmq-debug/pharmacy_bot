...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(control functionalities achievable through serial communication)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control, the sequence from inside to outside on the board is 1-4)
    ...
    PACKET_FUNC_SBUS = 9  # 航模遥控获取(obtained from remote control for model aircraft)
...
class Board:
    ...
    def set_motor_pwm(self, motor_id, duty_cycle):
        data = [0x20,]
        data.extend(struct.pack("<Bf", int(motor_id - 1), float(duty_cycle)))
        self.buf_write(PacketFunction.PACKET_FUNC_MOTOR, data)


    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(the sub-command is 0x01 to set the SSID. The second byte is the string length, including the '\0' string terminator)
    ...
    def bus_servo_read_and_unpack(self, servo_id, cmd, unpack):
        with self.servo_read_lock:
            ...
            data = self.bus_servo_queue.get(timeout=0.1)
                ...
                for dat in recv_data:
                    ...
                    if self.state == PacketControllerState.PACKET_CONTROLLER_STATE_STARTBYTE1:
                        ...
                        if crc8 == dat:
                            ...
                            # print("校验失败")(print('verification failed'))
                            pass
                        self.state = PacketControllerState.PACKET_CONTROLLER_STATE_STARTBYTE1
                        continue
        self.port.close()
